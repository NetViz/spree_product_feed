...
describe 'Tests A Product With Variants Added To The Feed', type: :feature, js: true do
  ...
  context 'Testing the compare price' do
    it "sets the sale price if the compare price is higher than selling price" do
      ...
      xml = Capybara.string(page.body)
      expect(xml).to have_text('<g:price>80.0 USD</g:price>')
      expect(xml).to have_text('<g:sale_price>50.0 USD</g:sale_price>')
    end
    ...
    it "does not set the sale price if the compare price is lower than selling price" do
      ...
      xml = Capybara.string(page.body)
      expect(xml).to have_text('<g:price>50.0 USD</g:price>')
      expect(xml).not_to have_text('<g:sale_price>80.0 USD</g:sale_price>')
    end
  ...
  context 'Test variant URL query', js: true do
    it 'shows price $50.00 for variant 4' do
      product.option_types << option_type
      product.variants << [variant1, variant2, variant3]
      product.master.stock_items.update_all count_on_hand: 10, backorderable: true
      product.tap(&:save)

      visit spree.product_path(product) + '?variant=4'
      expect(page).to have_content('$50.00')
    end

    it 'shows price $69.99 for variant 3' do
      product.option_types << option_type
      product.variants << [variant1, variant2, variant3]
      product.master.stock_items.update_all count_on_hand: 10, backorderable: true
      product.tap(&:save)

      visit spree.product_path(product) + '?variant=3'
